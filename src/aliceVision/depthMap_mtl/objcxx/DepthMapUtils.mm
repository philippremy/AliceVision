...
void copyFloat2Map(image::Image<float>& out_mapX,
                   ...
                   const std::unique_ptr<MTLDeviceMemory<float2, 2, MemoryType::HostVisible>>& in_map_hmh,
                   const ROI& roi,
                   int downscale)
{
    ...
    in_map_hmh->readOnHost([&](std::mdspan<const float2, std::dextents<size_t, 2>> contents) {
        for (int x = 0; x < width; ++x)
        {
            for (int y = 0; y < height; ++y)
            {
                const float2& value = contents[y, x];
...
void copyFloat2Map(image::Image<float>& out_mapX,
                   image::Image<float>& out_mapY,
                   const std::unique_ptr<MTLDeviceMemory<float2, 2, MemoryType::DeviceOnly>>& in_map_dmp,
                   const ROI& roi,
                   int downscale)
{
    // copy float2 map from device pitched memory to host memory
    std::unique_ptr<MTLDeviceMemory<float2, 2, MemoryType::HostVisible>> map_hmh =
      std::make_unique<MTLDeviceMemory<float2, 2, MemoryType::HostVisible>>();
    map_hmh->associateWithMTLDevice(in_map_dmp->getAssociatedDevice());
    map_hmh->allocate2D(in_map_dmp->getSize());

    map_hmh->copyFromOtherBuffer(*in_map_dmp);
                    ...
                    const std::string& name)
{
    ...
    std::unique_ptr<MTLDeviceMemory<float3, 2, MemoryType::HostVisible>> map_hmh =
      std::make_unique<MTLDeviceMemory<float3, 2, MemoryType::HostVisible>>();
    map_hmh->associateWithMTLDevice(in_map_dmp->getAssociatedDevice());
    map_hmh->allocate2D(in_map_dmp->getSize());
    ...
    map_hmh->readOnHost([&](std::mdspan<const float3, std::dextents<size_t, 2>> contents) {
        for (size_t x = 0; x < size_t(width); ++x)
        {
            for (size_t y = 0; y < size_t(height); ++y)
            {
                const float3& rgba_hmh = contents[y, x];
...
void writeDepthSimMap(int rc,
                      ...
                      const std::string& name)
{
    constexpr const mvsUtils::EFileType fileTypeX = mvsUtils::EFileType::depthMap;
    constexpr const mvsUtils::EFileType fileTypeY = mvsUtils::EFileType::simMap;
...
void resetDepthSimMap(std::unique_ptr<MTLDeviceMemory<float2, 2, MemoryType::HostVisible>>& inout_depthSimMap_hmh, float depth, float sim)
{
    const MTLResourceSize<2>& depthSimMapSize = inout_depthSimMap_hmh->getSize();

    inout_depthSimMap_hmh->writeFromHost([&](std::mdspan<float2, std::dextents<size_t, 2>> contents) {
        for (size_t x = 0; x < depthSimMapSize.width(); ++x)
        {
            for (size_t y = 0; y < depthSimMapSize.height(); ++y)
            {
                float2& depthSim_hmh = contents[y, x];

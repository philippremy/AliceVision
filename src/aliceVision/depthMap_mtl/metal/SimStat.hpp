...
struct simStat
{
    ...
    thread simStat& operator=(thread const simStat& param)
    {
        xxsum = param.xxsum;
        yysum = param.yysum;
        xysum = param.xysum;
     ...
     */
    float getVarianceXW() const { return (xxsum - xsum * xsum / wsum) / wsum; }

    /**
     * @brief Variance of Y
     * formula: sum(w*y*y) / sum(w) - sum(w*y)^2 / sum(w)^2
     */
    float getVarianceYW() const { return (yysum - ysum * ysum / wsum) / wsum; }
     ...
     */
    float getVarianceXYW() const { return (xysum - xsum * ysum / wsum) / wsum; }
    ...
    void update(thread const float2& g)
    {
        count += 1.0f;
        xsum += g.x;
        ysum += g.y;
    ...
    void update(thread const float2& g, float w)
    {
        wsum += w;
        count += 1.0f;
        xsum += w * g.x;
    ...
    void update(thread const float3& c1, thread const float3& c2)
    {
        float2 g;
        g.x = c1.x;
        g.y = c2.x;

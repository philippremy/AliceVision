...
if(AV_USE_CUDA)
    ...
    option(AV_BUILD_POPSIFT "Enable building an embedded PopSift" OFF)
endif()
...
if(APPLE)
    option(AV_BUILD_LAPACK "Enable building an embedded Lapack" OFF) # On Darwin targets, BLAS/LAPACK is provided by Accelerate.framework
    option(AV_BUILD_SUITESPARSE "Enable building an embedded SuiteSparse" OFF) # On Darwin targets, Sparse Solvers are provided by Accelerate.framework (only used by Ceres)
else()
...
if(AV_BUILD_OPENIMAGEIO)
    option(AV_BUILD_PYBIND11 "Enable building of pybind11 library" ON)
else()
...
if(AV_BUILD_SWIG)
    option(AV_BUILD_PCRE2 "Enable building an embedded PCR2 library" ON)
else()
    option(AV_BUILD_PCRE2 "Enable building an embedded PCR2 library" OFF)
endif()
...
message(STATUS "AV_BUILD_PCRE2 ${AV_BUILD_PCRE2}")
...
if(AV_BUILD_GEOGRAM)
    ...
    elseif(APPLE)
        ...
        elseif(CMAKE_OSX_ARCHITECTURES MATCHES "arm64")
            ...
            message(FATAL_ERROR "Encountered unsupported CMAKE_OSX_ARCHITECTURES value when trying to set VORPALINE_PLATFORM for Geogram! Supported architectures are x86_64 and aarch64/arm64.")
        endif()
...
if(AV_BUILD_FFMPEG)
    if(AV_BUILD_VPX)
        ...
        # This is currently required until libVPX properly supports macOS 26
        # Tahoe.
        ...
        ExternalProject_add(${VPX_TARGET}
            ...
            CONFIGURE_COMMAND
                ${VPX_APPLE_LDFLAGS}
                <SOURCE_DIR>/configure --prefix=<INSTALL_DIR> ${VPX_TOOLCHAIN_FLAG}
    ...
    if(APPLE)
        ...
        set(FFMPEG_CFLAGS -I<INSTALL_DIR>/include\ ${APPLE_ARCH_FLAGS}\ -Wl,-headerpad_max_install_names)   # ffmpeg does not include padding in the Mach-O header
        set(FFMPEG_LDFLAGS -L<INSTALL_DIR>/lib\ ${APPLE_ARCH_FLAGS}\ -headerpad_max_install_names)  # ffmpeg does not include padding in the Mach-O header
    else()
...
if(AV_BUILD_FLANN)
    ...
    ExternalProject_Add(${FLANN_TARGET}
        ...
        INSTALL_DIR ${CMAKE_INSTALL_PREFIX}
        CONFIGURE_COMMAND 
            ${CMAKE_COMMAND} -E env PKG_CONFIG_PATH="${CMAKE_INSTALL_PREFIX}/lib64/pkgconfig:${CMAKE_INSTALL_PREFIX}/lib/pkgconfig"
            ${CMAKE_COMMAND} 
            ${CMAKE_CORE_BUILD_FLAGS}
...
if(AV_BUILD_PCRE2)
    ...
    ExternalProject_Add(${PCRE2_TARGET}
        URL https://github.com/PCRE2Project/pcre2/archive/refs/tags/pcre2-10.46.tar.gz
        URL_HASH MD5=d23a93c740f6e53833835493835a769b
        DOWNLOAD_DIR ${BUILD_DIR}/download/${PCRE2_TARGET}
        PREFIX ${BUILD_DIR}
        BUILD_IN_SOURCE 0
        BUILD_ALWAYS 0
        UPDATE_COMMAND ""
        SOURCE_DIR ${CMAKE_CURRENT_BINARY_DIR}/${PCRE2_TARGET}
        BINARY_DIR ${BUILD_DIR}/${PCRE2_TARGET}_build
        INSTALL_DIR ${CMAKE_INSTALL_PREFIX}
        CONFIGURE_COMMAND ${CMAKE_COMMAND} ${CMAKE_CORE_BUILD_FLAGS}
        -DCMAKE_INSTALL_PREFIX:PATH=<INSTALL_DIR> <SOURCE_DIR>
        BUILD_COMMAND $(MAKE) -j${AV_BUILD_DEPENDENCIES_PARALLEL}
    ...
    set(PCRE2_CMAKE_FLAGS
        -DPCRE2_DIR=${CMAKE_INSTALL_PREFIX}/lib/cmake/pcre2
    )
endif()
...
if(AV_BUILD_ALICEVISION)
    ...
    set(AV_TOPLEVEL_FLAGS
        ...
        -DALICEVISION_USE_METAL=${AV_USE_METAL}
    )
    set(AV_MISC_FLAGS
        -DMINIGLOG=ON
    )
    ...
    ExternalProject_Add(aliceVision
        ...
        # Install Prefix and Source Folder
        -DCMAKE_INSTALL_PREFIX:PATH=<INSTALL_DIR>
        <SOURCE_DIR>
        BUILD_COMMAND $(MAKE) -j${AV_BUILD_DEPENDENCIES_PARALLEL}
        DEPENDS ${AV_DEPS}
